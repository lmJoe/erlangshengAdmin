    ...
    <div style="position: relative">
      ...
      <div class="titleNav" @click.right.prevent="mouseRight">
        <el-tag v-for="(item, index) in getAsideTitle"
                ...
                :closable="(item.title == 'APP概况')? false : true">
          {{item.title}}
        </el-tag>
      </div>
    ...
    methods: {
      ...
      * */
      toUrl(path, index) {
        ...
        window.LogList.checkLog('二郎神后台管理系统','页面Tab按钮','点击跳转',detailList);
        //跳转路由
    ...
    * */
      closeTag(title, index) {
        ...
        * */
        ...
        detailList.push(detailArr);
        detailList = JSON.stringify(detailList);
        window.LogList.checkLog('二郎神后台管理系统','页面Tab按钮','关闭',detailList);
      },
      /*
      * 左侧滚动
      * */
...
<style scoped>
  ...
  .titleNavBox {
    ...
    height: 42px;
    z-index: 200;
    box-shadow: 1px 2px 3px 0 #595959;
  }
