        ...
        <div class="pagetarge">
          ...
          <span class="arget">
            <div class="argetTxt" v-if="rightSelect==0">
              ...
              <p>≥3分钟人数：当天app的每个用户总使用时长中大于等于3min的用户数（包含所有用户）</p>
            </div>
      ...
      <div class="surveyContainer">
        <div class="surveyTop">
          <div class="surveyDataTop">
            <el-row :gutter="25">
              ...
              <el-col :span="2"><div class="grid-content bg-purple surveySort" v-if="rightSelect==0">活跃用户</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveySort" v-if="rightSelect==1">活跃账户</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveySort" v-if="rightSelect==0">新增用户</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveySort" v-if="rightSelect==1">新增账户</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveySort">启动次数</div></el-col>
          ...
          <div class="surveyDataBottom">
            <el-row :gutter="25" v-for="(item,index) in recentData" :key="index">
              <el-col :span="1.5"><div class="grid-content bg-purple surveyDatatitle">昨日</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveyDataNum" v-if="rightSelect==0">{{item.activeUser}}</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveyDataNum" v-if="rightSelect==1">{{item.activeAccount}}</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveyDataNum" v-if="rightSelect==0">{{item.newUser}}</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveyDataNum" v-if="rightSelect==1">{{item.newAccount}}</div></el-col>
              <el-col :span="2"><div class="grid-content bg-purple surveyDataNum">{{item.startCount}}</div></el-col>
              ...
              <el-col :span="3"><div class="grid-content bg-purple surveyDataNum">{{item.pv}}</div></el-col>
          ...
          <div class="surveyEchartBox">
            <div class="surveyEchartlist">
              <el-tabs v-model="activeName" @tab-click="changeEchartBtn">
                ...
                <el-tab-pane label="≥3分钟人数" name="6">
                  <div id="echart6" style="height:400px;"></div>
                </el-tab-pane>
                <el-tab-pane label="（电商）链接点击次数" name="7">
                  <div id="echart7" style="height:400px;"></div>
                </el-tab-pane>
                <el-tab-pane label="（电商）链接点击人数" name="8">
                  <div id="echart8" style="height:400px;"></div>
                </el-tab-pane>
              </el-tabs>
            </div>
        ...
        <div class="surveyDetailBox">
          <div class="DetailBoxTop">
            ...
            <a class="surveyDown" id="surveyDown" @click="downloadE()">下载数据</a>
          </div>
  ...
  methods: {
    ...
    getAppRecentData (){
      ...
      http({
        ...
        method: 'get',
        url:URL.recordUrl.getbdgRecentData+"?SourceType="+that.rightSelect,
        data: {},
    ...
    getPartData (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getbdgPartData+"?partnum="+that.activeName+"&startTime="+that.echartDefaultDate[0]+"&endtime="+that.echartDefaultDate[1]+"&SourceType="+that.rightSelect,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getDetails (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getbdgDetails+"?startTime="+that.detailBoxDate[0]+"&endtime="+that.detailBoxDate[1]+"&SourceType="+that.rightSelect,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    //获取表格导出
    downloadE(){
      // this.downloadExcelUrl = URL.recordUrl.getbdgExport+"?startTime="+this.detailBoxDate[0]+"&endtime="+this.detailBoxDate[1]+"&SourceType="+this.rightSelect;
      var url = URL.recordUrl.getbdgExport;
      var thedata = 'bearer'+' '+ localStorage.getItem("accessToken");
//       var thedata = "Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6Ii1PZUVVR0NfZlFqa0U3b0dZZHhrX0EiLCJ0eXAiOiJhdCtqd3QifQ..RPm-tjBtNzOuSrVvgStJtV0yXTQUEHNWt6jCb1-VX3_UfNNu-AHbv92Vbrlf-XUwdbc1N6nOjSrdBy3Gb0TzDKB61Atp_-_nGU05SUq1iAmmTttGPjz2jkDA4QWTTxONIVRilsHELMigZSNIJErcpg8cZE560Yt6U_vMqFZzmlN_VhfMyHWtpjPupvldVSG4k-i0p7puU7DrBrLFoM-WQCCKp-oxwObQYWMz3EFR66Ancc6Kf2T-l9CipKps7UBmBvjg0DAwO9QyHB80LkbzUmRGYNBbGsRyALpO9T4fq0kCqFLOlZF3JZrCUW0IkvETLJBXvpwP3gll95bXuywI5Q";
      let myObj = {
        method: 'get',
        url:url,
        fileName: '',
        params: `startTime=${this.detailBoxDate[0]}&endtime=${this.detailBoxDate[1]}&SourceType=${this.rightSelect}`,
        thedata:thedata,
      }
