...
export default {
  ...
  methods: {
    ...
    bindDrawal(){
      ...
      //获取右上角筛选下拉菜单
      this.getSelectList();
      this.getPartData();//获取总趋势图数据
      this.getDetails();//表格数据
      this.getPartDataBF();//获取播放视频趋势图
      this.getPlayTypeBF();//分布柱状图
      this.getmoheVideoPlayTop();//视频播放top10
    ...
    getAppRecentData (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getmoheGeneralData+"?MediaName="+that.MediaName+'&ChannelName='+that.ChannelName+'&CompanyName='+that.CompanyName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
        ...
        that.loading = false;
        that.playTime = res.data[0].playTime;
        that.playNum = res.data[0].playNum;
        that.playVideoCount = res.data[0].playVideoCount;
        that.watchNum = res.data[0].watchNum;
        that.complete = res.data[0].complete;
        that.playVideoCountPre = res.data[0].playVideoCountPre;//平均播放视频数
        that.playNumPre = res.data[0].playNumPre;//平均播放量
        that.playTimePre = res.data[0].playTimePre;//平均播放时长
      }).catch(function (err) {
        console.log(err);
      });
    },
    ...
    changeEchartBtn1() {
      ...
      detailList.push(detailArr);
      detailList = JSON.stringify(detailList);
      window.LogList.checkLog('二郎神后台管理系统',this.title,'视频播放趋势图左侧分类条件选择',detailList);
    },
    changeEchartBtn2() {
      ...
      this.getPartDataBF();//视频播放趋势
      var detailList = [];
      var detailArr = [];
      detailArr[0] =  this.Valuename;
      detailList.push(detailArr);
      detailList = JSON.stringify(detailList);
      window.LogList.checkLog('二郎神后台管理系统',this.title,'视频播放趋势图右侧分类条件选择',detailList);
    },
    //获取总趋势图数据
    getPartData (){
      var that = this;
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getmoheVideoCount+"?CompanyName="+that.CompanyName+"&MediaName="+that.MediaName+'&starttime='+that.echartDefaultDateAll[0]+'&endtime='+that.echartDefaultDateAll[1]+'&ChannelName='+that.ChannelName+'&VideoPlayType='+that.tabPositionAll,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getPartDataBF (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getmoheVideoPlayTrend+"?ChannelName="+that.ChannelName+"&VideoPlayBehavior="+that.tabPosition1+'&starttime='+that.echartDefaultDate[0]+'&endtime='+that.echartDefaultDate[1]+'&VideoPlayBehaviorType='+that.tabPosition2+'&CompanyName='+that.CompanyName+'&MediaName='+that.MediaName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getPlayTypeBF (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getmoheVideoTypeData+"?ChannelName="+that.ChannelName+'&MediaName='+that.MediaName+'&CompanyName='+that.CompanyName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getmoheVideoPlayTop() {
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getmoheVideoPlayTop+"?MediaName="+that.MediaName+"&ChannelName="+that.ChannelName+'&CompanyName='+that.CompanyName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getDetails (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getmoheVideoPlayDetails+"?MediaName="+that.MediaName+"&ChannelName="+that.ChannelName+"&starttime="+that.detailDefaultDate[0]+"&endtime="+that.detailDefaultDate[1]+'&CompanyName='+that.CompanyName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    changedetailTime() {
      ...
      detailList.push(detailArr);
      detailList = JSON.stringify(detailList);
      window.LogList.checkLog('二郎神后台管理系统',this.title,'表格数据时间切换',detailList);
    },
    //实现趋势图
...
<style scoped lang="less">
 @import '../../assets/css/mohePlayData.less';
