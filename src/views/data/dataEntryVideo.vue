  ...
  methods: {
    ...
    BolgList() {
     var that = this;
      http({
        ...
        method: 'get',
        url:URL.recordUrl.getVdLog+"?StartTime="+that.blogBoxDate[0]+"&EndTime="+that.blogBoxDate[1],
        data: {},
    ...
    changeMedia(value){
      ...
      http({  
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.GetVdOld+'?DateId='+that.echartDefaultDate.dateValue+'&CId='+that.mediaValue,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
    ...
    detailData(){
      ...
      http({  
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getVdData+"?startTime="+that.detailBoxDate[0]+"&endtime="+that.detailBoxDate[1]+"&Page="+that.pageIndex+"&Rows=30",
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
