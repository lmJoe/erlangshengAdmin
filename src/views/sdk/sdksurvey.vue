        ...
        <div class="surveyTop">
          <div class="surveyDataTop">
            <el-row :gutter="25">
              ...
              <el-col :span="3"><div class="grid-content bg-purple surveySort">活跃用户</div></el-col>
...
export default {
  ...
  data () {
    return {
      ...
      loading: true,
      activeName: '1',//趋势图切换按钮值 1-活跃用户 2-活跃账户 1-新增用户 2-新增账户 3-启动次数  4-人均播放量 5-人均使用时长
  ...
  methods: {
    ...
    getAppRecentData (){
      ...
      http({
        ...
        method: 'get',
        url:URL.recordUrl.getsdkRecentData+"?MediaName="+that.MediaName+"&CompanyName="+that.CompanyName,
        data: {},
    ...
    getPartData (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getsdkPartData+"?MediaName="+that.MediaName+"&partnum="+that.activeName+"&startTime="+that.echartDefaultDate[0]+"&endtime="+that.echartDefaultDate[1]+"&CompanyName="+that.CompanyName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getDetails (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getsdkDetails+"?MediaName="+that.MediaName+"&startTime="+that.detailBoxDate[0]+"&endtime="+that.detailBoxDate[1]+"&CompanyName="+that.CompanyName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    //获取表格导出
    downloadE(){
      this.downloadExcelUrl = URL.recordUrl.getsdkExport+"?MediaName="+this.MediaName+"&CompanyName="+this.CompanyName+"&startTime="+this.detailBoxDate[0]+"&endtime="+this.detailBoxDate[1];
      // var detailList = [];
      // var detailArr = [];
      // detailArr[0] =  this.MediaName;
      // detailArr[1] =  this.CompanyName;
      // detailArr[2] =  this.detailBoxDate[0];
      // detailArr[3] =  this.detailBoxDate[1];
      // detailList.push(detailArr);
      // detailList = JSON.stringify(detailList);
