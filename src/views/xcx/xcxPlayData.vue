  ...
  <div class="appSurvey" v-loading="loading">
    <div class="appSurveyContainer">
      <div class="headContainer">
        <div class="pagetarge">
          <span class="">小程序播放数据</span>
          <span class="arget">
            <div class="argetTxt">
              <h3>指标：</h3>
              <p>播放时长：当天小程序的播放视频总时长</p>
              <p>播放量：当天小程序的视频总播放量</p>
              <p>播放视频数：当天小程序的视频播放个数</p>
              <p>观看视频人数：当天小程序的播放视频的总用户数</p>
              <p>播放完成度：当天小程序的视频播放时长与视频总时长比的平均值</p>
              <p>平均用户播放时长：当天小程序的播放视频总时长与播放视频的总用户数比</p>
              <p>平均用户播放量：当天小程序的视频总播放量与播放视频的总用户数比</p>
              <p>平均用户视频播放数：当天小程序的视频播放个数与播放视频的总用户数比</p>
            </div>
          </span>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      rightName: 43,//右上角选项类型值 0-M站 4-爆米花PC
      loading: true,
  ...
  methods: {
    ...
    getAppRecentData (){
      ...
      http({
        ...
        method: 'get',
        url:URL.recordUrl.getxcxPlayGeneralData+"?AppId="+that.rightName,
        data: {},
    ...
    getPartData (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getxcxGetVideoCount+"?AppId="+that.rightName+"&VideoPlayType="+that.tabPositionAll+'&starttime='+that.echartDefaultDateAll[0]+'&endtime='+that.echartDefaultDateAll[1],
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getPartDataBF (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getxcxVideoPlayTrend+"?AppId="+that.rightName+"&VideoPlayBehavior="+that.tabPosition1+'&starttime='+that.echartDefaultDate[0]+'&endtime='+that.echartDefaultDate[1]+'&VideoPlayBehaviorType='+that.tabPosition2,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getPlayTypeBF (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getxcxPlayVideoTypeData+"?AppId="+that.rightName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getAppVideoPlayTop() {
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getxcxVideoPlayTop+"?AppId="+this.rightName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getDetails (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getxcxPlayGetDetails+"?AppId="+that.rightName+"&starttime="+that.detailDefaultDate[0]+"&endtime="+that.detailDefaultDate[1],
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
...
<style scoped lang="less">
 @import '../../assets/css/xcxPlayData.less';
