          ...
          <div class="surveyEchartBox">
            <div class="surveyEchartlist">
              <el-tabs v-model="activeName" @tab-click="changeEchartBtn">
                ...
                <el-tab-pane label="≥30分钟人数" name="3">
  ...
  methods: {
      ...
      http({
        ...
        that.options.forEach((item,index)=>{
          ...
          if(index==0){
            ...
            that.rightName = item.value;
            that.getAppRecentData();//获取APP概况数据
            that.getPartData();
            that.getDetails();
          }
          if(item.label==null){
    ...
    getAppRecentData (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getsdkTimeGeneralData+"?CompanyName="+that.CompanyName+"&MediaName="+that.MediaName,
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getPartData (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getsdkTimePartData+"?CompanyName="+that.CompanyName+"&partnum="+that.activeName+"&MediaName="+that.MediaName+"&startTime="+that.echartDefaultDate[0]+"&endtime="+that.echartDefaultDate[1],
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
    ...
    getDetails (){
      ...
      http({
        //这里是你自己的请求方式、url和data参数
        method: 'get',
        url:URL.recordUrl.getsdkTimeDetailsData+"?CompanyName="+that.CompanyName+"&MediaName="+that.MediaName+"&startTime="+that.detailBoxDate[0]+"&endtime="+that.detailBoxDate[1],
        data: {},
        //假设后台需要的是表单数据这里你就可以更改
        headers: {
          "Content-Type":"application/x-www-form-urlencoded",
